...
def _newton_delay(signal, w0, epsilon, max_iters, discard_unconverged):
    ...
    while np.any(active) and iteration < max_iters:
        ...
        with np.errstate(divide='ignore', invalid='ignore'):
            ...
            # note objective function is actually C = X*X.conj()
            X = temp.mean(axis=-1)
            np.multiply(temp, njx, out=temp)
            dX = temp.mean(axis=-1)
            np.multiply(temp, njx, out=temp)
            d2X = temp.mean(axis=-1)
            # from dC = X*dX.conj())+X.conj()*dX = 2 * (X * dX.conj()).real
            halfdC = (X * dX.conj()).real
            # originally d2C=(X*d2X.conj() + X.conj()*d2X + 2*dX*dX.conj()).real
            # which is real; just drop imag=0 component
            halfd2C = (X * d2X.conj()).real + (dX * dX.conj()).real
            # newton update
            np.divide(halfdC, halfd2C, out=delta)
        delta[np.isnan(delta)] = 0.0
        active = ~np.isinf(delta) & (np.abs(delta) >= epsilon)
...
def fft_newton(signal, NFFT=None, epsilon=1e-10, max_iters=100,
               discard_unconverged=False):
